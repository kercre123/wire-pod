...
if [[ ${STT_SERVICE} == "leopard" ]]; then
	if [[ -f ./chipper ]]; then
	  ...
	  /usr/local/go/bin/go run cmd-leopard/main.go -tags leopard
	fi
	...
	if [[ -f ./chipper ]]; then
		...
		/usr/local/go/bin/go run cmd-vosk/main.go -tags vosk
	fi
else
if [[ -f ./chipper ]]; then
  ...
  /usr/local/go/bin/go run cmd/main.go -tags coqui
fi
fi
