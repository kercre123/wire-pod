...
if [[ ${STT_SERVICE} == "leopard" ]]; then
	if [[ -f ./chipper ]]; then
	  ...
	  /usr/local/go/bin/go run -tags ${STT_SERVICE} cmd-leopard/main.go
	fi
	...
	if [[ -f ./chipper ]]; then
		...
		/usr/local/go/bin/go run -tags ${STT_SERVICE} cmd-vosk/main.go
	fi
else
if [[ -f ./chipper ]]; then
  ...
  ./chipper
      ...
      /usr/local/go/bin/go run -tags ${STT_SERVICE} cmd/main.go
fi
fi
